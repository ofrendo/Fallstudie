...
public class TestBuildingFinished extends AbstractClientServerTest {

	@Test
	public void testBuildingFinished() {
		...
		assertEquals(ResourceRegionStatus.POWERSTATION, region.resourceRegionStatus);
		...
		for (int i = 0; i<region.resourceType.mBuildTime+1; i++) {
			//Then send ready again
			client1.sendReadyMessage(true);
			client2.sendReadyMessage(true);
		...
		assertEquals(ResourceRegionStatus.MINE_POWERSTATION, region.resourceRegionStatus);
