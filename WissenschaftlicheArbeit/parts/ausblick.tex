%Matthias
\section{Ausblick}\label{matthias:ausblick}
Ein wichtiges Ziel dieser Arbeit war das eigene Organisieren und Erfahren eines
kompletten Projekts des Software Engineering.\\
Der Ablauf mit Planungsphase, Analyse, Design, Entwurf sowie das Arbeiten mit
standardisierten Modellen wie den UML-Notationen haben wir am eigenen Leib
erfahren. Auch die negativen Aspekte wie Fehlplanung und Zeitdruck haben wir
erlebt, ohne die sich ein Projekt vermutlich gar nicht realisieren lässt.\\
Unter Anwendung von statischen und dynamischen Modellen haben wir als Team an
einem Stück Software entwickelt, was sich trotz (oder gerade wegen)
Hilfsprogrammen wie Git nicht immer als einfach erwiesen hat.
Vor allem eine sinnvolle Aufgabenteilung fiel zu Beginn schwer, Absprachen
wurden teilweise gar nicht erst getroffen, Missverständnisse führten zu
langwierigen Diskussionen.\\
Doch nachdem sich alle über die grundlegenden Ideen einig waren und der
Zeitdruck die Lust auf eben jene Diskussionen schwinden ließ, war ein klarer
Anstieg der Produktivität zu bemerken und das Gefühl, zusammen und nicht jeder
für sich an dem Coding zu arbeiten, stellte sich langsam ein, auch wenn
sich immer wieder technische Schwierigkeiten wie Konflikte in Git oder
plattformabhängige Fehler bei der Darstellung des GUIs einstellten.

Schließlich haben wir es aber geschafft, zur Abgabe ein lauffähiges
Unternehmensplanspiel mit vielen Features und einem funktionierenden User
Interface abzugeben.\\
Gerade im Bereich des Projektmanagements aber auch in technischen Bereichen wie
dem kollaborativen Programmieren an einer Software haben wir viele Erfahrungen
gesammelt, die man in einer gewöhnlichen Vorlesung so nicht hätte erfahren
können.

Abschließend lässt sich definitiv behaupten, dass wir viel gelernt haben und
auch der Spaß zwischendurch nicht zu kurz kam.

% Wordcount: 250